...
def strip_trailing_tool_use(messages: object) -> list[dict[str, object]]:
    ...
    while messages:
        ...
        if not isinstance(original_content, list) or not all(
            ...
            error = "each message content value must be a list of content blocks"
...
def _extract_prompt(payload: dict):
    ...
    if "tool_results" in payload:
        ...
        if not isinstance(tool_results, list) or not all(
            ...
            error = "tool_results must be a list of objects with a toolUseId string"
            raise ValueError(error)
        return [
            {
                "role": "user",
